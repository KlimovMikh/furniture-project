...
def remove_prefixes_and_postfixes(strings):
    # Remove duplicates to avoid redundant checks
    unique_strings = set(strings)
    # Set to hold strings that are prefixes or postfixes of others
    ...
    # Subtract the substrings to remove from the unique strings
    result = unique_strings - substrings_to_remove
    return list(result)
...
if __name__ == "__main__":
    ...
    if 'error' in processed[0]:
        print(json.dumps("Error: " + processed[0].get('error')))
    else:
        ...
        labels = remove_prefixes_and_postfixes(labels)
